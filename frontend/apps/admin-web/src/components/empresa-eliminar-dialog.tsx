...
export function EmpresaEliminarDialog({
  ...
  return (
    <div
      className={cn(platformUi.modalOverlay, "items-start overflow-y-auto overscroll-contain py-6 sm:py-10")}
      role="presentation"
      onClick={(e) => {
    ...
    >
      <div
        className={cn(
          platformUi.modal,
          "my-auto flex max-h-[min(calc(100vh-3rem),720px)] w-full max-w-lg flex-col"
        )}
        role="alertdialog"
        aria-modal="true"
      ...
      >
        <div className="platform-modal-header shrink-0 !border-b-0 !pb-2">
        ...
        <div className="platform-modal-body min-h-0 flex-1">
          {resumenLoading && (
            ...
            <div className="space-y-3">
              ...
              {filas.length === 0 ? (
                <p className="text-sm text-slate-600">
                  No hay registros operativos; solo se eliminará la ficha de empresa y tenants.
                </p>
              ) : (
                <ul className="rounded-xl border border-slate-200 bg-slate-50/80 px-3 py-2 text-sm">
                  {filas.map((f) => (
                    <li
                      key={f.label}
                      className="flex justify-between gap-4 border-b border-slate-200/80 py-1.5 last:border-0"
                    >
        ...
        <div className="platform-modal-footer shrink-0">
